"""
app/data/help_content.py

Purpose:
    Python mirror of frontend/src/constants/help-content.ts. Backend Ask AI
    retrieval (Phase 2 — see app/api/help.py::ask_ai) reads from this dict
    to assemble the top-N article context block fed into Gemini.

    This is INTENTIONALLY a hand-mirror of the TypeScript file rather than
    a build-time parse, because (a) the TS file is small (<25 KB), (b) it
    changes infrequently outside of Phase 3 when the backend CMS takes over,
    and (c) a build-time parser would add fragility for a temporary file.

    Phase 3 will replace this module with a database read against the
    help_articles table. When that lands, this file gets deleted.

Responsibilities:
    - Single in-memory dict mirroring HELP_CONTENT.
    - Flat list of (title, body, page_key, role) tuples for retrieval scoring.

Critical invariants:
    - Content MUST stay in product voice — no Vue component names, no DB
      schema terms, no phase markers, no HTTP routes, no env var names.
      (Mirror of the rule in the .ts file's header.)
    - When the .ts file is edited in Phase 1 or beyond, this file MUST be
      kept in sync until Phase 3 retires it. The single test
      tests/test_help_ask.py::test_corpus_mirrors_typescript reads a sample
      of titles from both sides and asserts equality.

Plan: docs/plans/2026-06-05-009-help-center-povin-pixel-port-plan.md §7.1
"""
...
class _HelpTopic(TypedDict):
    q: str
    a: str


class _HelpEntry(TypedDict):
    intro: str
    topics: list[_HelpTopic]


class _HelpPage(TypedDict, total=False):
    title: str
    all: _HelpEntry
    user: _HelpEntry
    admin: _HelpEntry
...
class _HelpContentShape(TypedDict):
    pages: dict[str, _HelpPage]
    faq: list[_HelpTopic]
    contact: dict[str, str]


HELP_CONTENT: _HelpContentShape = {
    "pages": {
        "dashboard": {
            "title": "Dashboard",
            "user": {
...
class HelpArticle(TypedDict):
    """One retrievable article — flattened from HELP_CONTENT.

    id: stable string id (page_key:role:topic_index OR faq:index).
    page_key: e.g. 'editor' (or '' for FAQ).
    role: 'user' | 'admin' | '' for FAQ.
    title: the question text.
    summary: the answer text (used as the RAG context block per article).
    """
    id: str
    page_key: str
    role: str
...
def flatten_corpus() -> list[HelpArticle]:
    """Return the full searchable corpus as a flat list of HelpArticle dicts.

    Order: pages × roles (in HELP_CONTENT iteration order), then FAQ.
    Stable ids: '<page_key>:<role>:<topic_idx>' OR 'faq:<idx>'.
    """
    out: list[HelpArticle] = []
    for page_key, page in HELP_CONTENT["pages"].items():
        for role in ("all", "user", "admin"):
            entry: Optional[_HelpEntry] = page.get(role)  # type: ignore[assignment]
            if not entry:
                continue
            for idx, t in enumerate(entry["topics"]):
                out.append({
                    "id": f"{page_key}:{role}:{idx}",
                    "page_key": page_key,
                    "role": role,
                    "title": t["q"],
                    "summary": t["a"],
                })
    for idx, t in enumerate(HELP_CONTENT["faq"]):
        out.append({
            "id": f"faq:{idx}",
            "page_key": "",
            "role": "",
