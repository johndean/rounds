/**
 * frontend/src/constants/help-content.ts
 *
 * Hand-authored Phase 1 Help Center baseline for rounds.vin. Drafted in
 * product voice for end users + admins. Structure mirrors po.vin's
 * help-content.ts so future phases (CMS backend, AI bulk rewrites,
 * version history) can layer overrides on top without reshaping the
 * consumer components.
 *
 * CRITICAL CONTENT RULES (do not break when editing):
 *   - Plain language. No backend implementation details.
 *   - No Vue component names (e.g. "SpeakerEditPanel"), no DB schema
 *     terms ("text_edit ledger row", "audit_events"), no phase markers
 *     ("Phase 9.5 Layer 1"), no HTTP routes ("GET /v1/sessions/{id}"),
 *     no env var names ("VITE_HELP_ASK_AI_ENABLED").
